...
interface ProgramSettingsStepProps {
  form: UseFormReturn<z.infer<typeof programSchema>>;
}

const DIFFICULTY: ProgramDifficulty[] = ['초보자', '중급자', '숙련자'];

export default function ProgramSettingsStep({ form }: ProgramSettingsStepProps) {
  ...
  const handleChangeDifficult = useCallback(
    (value: ProgramDifficulty | undefined) => {
  ...
  return (
    <div className="w-full flex-shrink-0">
      <Card className="border-0 bg-white/80 shadow-lg backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="text-center text-gray-900">프로그램 설정</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="rounded-lg border border-gray-200 bg-white px-4">
            <div className="flex items-center justify-between pt-4">
              <div className="space-y-1">
                <Label className="text-base font-semibold text-gray-900">공개 프로그램</Label>
                <p className="text-sm text-gray-600">다른 사용자들과 프로그램을 공유합니다</p>
              </div>
              <Switch
          ...
          <div className="space-y-2">
            <Label htmlFor="description">프로그램 설명 (선택사항)</Label>
            <Textarea
              id="description"
              placeholder="이 프로그램에 대한 설명을 작성해보세요..."
              rows={4}
