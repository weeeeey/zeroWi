...
import {
  ProgramBasicInfoStep,
  ProgramExerciseConfigStep,
  ProgramSettingsStep,
} from '@/components/features/programs/create';
import CreateAlertModal from '@/components/features/programs/create/create-alert-modal';
...
import { SEARCHPARAM_PROGRAMID } from '@/lib/programs/constant';
...
export default function ProgramCreatorPage() {
  ...
  // 3단계에서 프로그램 생성하기 버튼(타입 submit) 클릭시 form submit 실행
  const onSubmit = async (data: z.infer<typeof programSchema>) => {
    try {
      ...
      if (parsingResponse.success) {
        router.push(`/programs?${SEARCHPARAM_PROGRAMID}=${parsingResponse.programId}`);
        setTimeout(() => {
  ...
  return (
    <div className="container space-y-6">
      ...
      <div ref={headerRef} className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900">프로그램 생성</h1>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <div className="overflow-hidden">
          ...
          >
            {/* 1단계: 프로그램 이름,타입(싱글 or 멀티) */}
            <ProgramBasicInfoStep form={form} />
            {/* 2단계: 프로그램 일자별 운동과 각 세트별 셋팅 */}
            <ProgramExerciseConfigStep />
            {/* 3단계: 공유 or 비공유 선택 및 최종 컨펌 */}
            <ProgramSettingsStep form={form} />
          </div>
        </div>
      </form>
